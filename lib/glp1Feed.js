...
/** Domains fetched via OpenAI web search (STAT covered by RSS). */
...
function buildDomainPrompt(domain, sourceName, maxPerDomain) {
  return (
    'Search ' +
    domain +
    ' for up to ' +
    maxPerDomain +
    ' recent GLP-1 or obesity drug news stories from the last ' +
    MAX_AGE_DAYS +
    ' days.\n\n' +
    'Topics: GLP-1, Ozempic, Wegovy, Mounjaro, tirzepatide, semaglutide, Zepbound, obesity drugs, FDA actions, trials, insurance coverage.\n' +
    'Include only real news — not explainers, tips, guides, or clickbait.\n\n' +
    'Return ONLY valid JSON (no markdown):\n' +
    '{"items":[{"title":"headline","href":"https://' +
    domain +
    '/...","date":"Jun 28, 2026","source":"' +
    sourceName +
    '","searchTerm":"' +
    sourceName +
    '"}]}'
...
async function fetchOpenAiForDomain(domain, maxPerDomain) {
  const apiKey = getOpenAiKey();
  const sourceName = sourceFromDomain('https://' + domain + '/', domain);
  ...
  try {
    const response = await fetch(OPENAI_RESPONSES_URL, {
      ...
      body: JSON.stringify({
        ...
        input: buildDomainPrompt(domain, sourceName, maxPerDomain),
      }),
    ...
    return collectOpenAiRawItems(payload);
  } finally {
    clearTimeout(timer);
  }
}
...
async function fetchOpenAiItems(maxItems) {
  ...
  const perDomain = Math.max(1, Math.min(3, Math.ceil(maxItems / OPENAI_SEARCH_DOMAINS.length)));
  const results = await Promise.allSettled(
    OPENAI_SEARCH_DOMAINS.map(function (domain) {
      return fetchOpenAiForDomain(domain, perDomain);
    })
  );
  ...
  for (const result of results) {
    if (result.status === 'rejected') {
      console.warn('GLP-1 OpenAI domain search failed:', result.reason && result.reason.message);
      continue;
    }
