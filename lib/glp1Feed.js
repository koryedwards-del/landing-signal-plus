...
/** Subscription, registration, or login required to read the full article */
...
const DISCOVERY_QUERIES = [
  ...
  'semaglutide site:reuters.com',
  'GLP-1 site:apnews.com',
  'Ozempic site:apnews.com',
  'GLP-1 site:npr.org',
  'GLP-1 FDA site:fda.gov',
  'obesity drug site:fda.gov',
];
...
function buildOpenAiPrompt(limitPerTerm, maxItems) {
  return (
    ...
    MAX_AGE_DAYS +
    ' days.\n\n' +
    'Prefer free-to-read articles on: reuters.com, fda.gov, npr.org, apnews.com.\n' +
    'Exclude any publisher that requires subscription, registration, or login (e.g. STAT, Medscape), plus CNBC, Healthline, GoodRx, Forbes, and Google News links.\n\n' +
...
async function fetchDiscoveryItems(limitPerTerm) {
  const perQuery = Math.max(3, Math.min(limitPerTerm, 5));
...
async function buildFeed(limitPerTerm) {
  const maxItems = Math.max(16, Math.min(TOP_TERMS_COUNT * limitPerTerm, MAX_FEED_ITEMS));
  ...
  try {
    ...
    console.warn('GLP-1 discovery failed:', err.message);
  }
